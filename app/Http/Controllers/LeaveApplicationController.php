...
class LeaveApplicationController extends Controller
{
    ...
    public function create(): View|RedirectResponse
    {
        ...
        $approvers = User::query()
            ...
            ->where(function ($query) {
                $query->whereIn('role', ['admin', 'manager', 'department_head'])
                    ->orWhere('can_approve_leave_requests', 1);
            })
            ->orderBy('email')
            ->get();
        return view('leaves.apply',['employee'=>$employee,'leaveTypes'=>$leaveTypes,'policyMap'=>$policies,'approvers'=>$approvers]);
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated=$request->validate(['leave_type_id'=>['required','integer','exists:leave_types,id'],'approver_user_id'=>['nullable','integer','exists:users,id'],'filing_date'=>['required','date'],'start_date'=>['required','date'],'end_date'=>['required','date'],'reason'=>['nullable','string'],'leave_subtype'=>['nullable','string','max:100'],'details'=>['nullable','array'],'details.*'=>['nullable'],'supporting_documents'=>['nullable','array'],'supporting_documents.*'=>['nullable'],'medical_certificate_attached'=>['nullable'],'affidavit_attached'=>['nullable'],'emergency_case'=>['nullable'],'commutation'=>['nullable','string','max:50'],'attachments.*'=>['nullable','file','max:10240']]);
