...
@push('head')
<style>
.leave-requests-shell{display:flex;flex-direction:column;gap:18px}.filters-card{padding:20px 22px}.filter-actions,.form-grid .field.filter-actions{display:flex!important;flex-direction:row!important;justify-content:flex-start;align-items:center;gap:10px;flex-wrap:nowrap;align-self:end}.filter-actions .btn,.form-grid .field.filter-actions .btn{width:auto!important;white-space:nowrap;margin:0}.section-intro{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap}.section-intro p{max-width:840px;margin:0}.requests-count-pill{display:inline-flex;align-items:center;gap:8px;padding:10px 14px;border-radius:999px;background:#eff6ff;border:1px solid #bfdbfe;color:#1d4ed8;font-weight:700}.request-list{display:flex;flex-direction:column;gap:14px}
...
.request-accordion-finalized .request-summary-row{cursor:default;pointer-events:auto}.request-accordion-finalized[open]{pointer-events:auto}.request-accordion-finalized summary{pointer-events:none}.request-accordion-finalized .summary-action-buttons *,.request-accordion-finalized .request-summary-toggle{pointer-events:auto}@media (max-width: 860px){.request-actions,.request-actions .inline-inputs,.print-form-grid{grid-template-columns:1fr}.filter-actions,.form-grid .field.filter-actions{justify-content:flex-start;flex-direction:row!important}.filter-actions .btn,.form-grid .field.filter-actions .btn{width:auto!important}.ls-modal{padding:10px}.ls-modal-dialog{width:100%;max-height:calc(100vh - 20px);padding:18px}.request-summary-row{grid-template-columns:1fr;align-items:flex-start}}
...
@include('partials.page-header', [
    ...
    'subtitle' => 'Review, track, and manage employee leave submissions',
    'actions' => [],
    'inline_count' => $rows->total()
])
<div class="leave-requests-shell">
    ...
    <div class="ui-card filters-card">
        <form method="GET" action="{{ route('leave.requests') }}" class="form-grid filter-grid" id="leaveRequestFilterForm">
            ...
            <div class="field"><label>Year</label><select name="year">
                <option value="0" @selected(request('year',0)==0)>All Years</option>
            ...
            <div class="field filter-actions" style=" padding-bottom: 20px; display:flex;flex-direction:row;align-items:center;justify-content:flex-start;gap:10px;flex-wrap:nowrap;align-self:end;"><button class="btn btn-action-slate" type="submit">Apply Filters</button><a href="{{ route('leave.requests', ['tab' => $tab]) }}" class="btn btn-action-light">Clear</a></div>
        </form>
    </div>
    ...
    <div class="request-list">
        @forelse($rows as $row)
            @php
                ...
                $form = $row->form;
            @endphp
            <details class="request-accordion ui-card{{ ($row->workflow_status === 'finalized' || $row->status === 'approved') ? ' request-accordion-finalized' : '' }}">
                <summary class="request-summary-row">
                    ...
                    <div class="request-summary-toggle">View Details <span class="summary-chevron">▾</span></div>
                </summary>
                        ...
                        <div class="request-overview">
                            <div class="request-person">
                                ...
                                <div class="summary-action-buttons">
                                    ...
                                    <button type="button" class="btn btn-primary" onclick="event.preventDefault();openLeaveModal('{{ $modalId }}')">View Full Request Details</button>
